...
- 화학 단원 문제는 2022 개정 2026판 완자 물질과 에너지 원본의 개념·대표 자료 유형을 근거로 새 문항을 만든다. 원문 문장을 복사하지 않는다.
- 역학과 에너지 완자 원본 PDF가 실제로 확보되지 않은 동안 물리 단원 문제를 “완자 원문 변형”으로 표시하지 않는다. 정동고 공식 범위와 학습 개념 기반 변형이라고 명시한다.
...
## 3. 단원 문제 흐름 계약
- **학습 흐름은 반드시 `단원 개념 확인 완료 → 그 단원 문제 → 오답 보강 완료 → 다음 단원` 순서다.**
- 이전 단원의 개념 확인만 끝났다고 다음 단원을 자동으로 열지 않는다. 이전 단원의 unit practice까지 완료해야 다음 단원이 열린다.
- 홈의 문제 진입은 전범위 실전이 아니라 `배운 단원 문제`가 기본이다.
- unit-practice.html은 개념 확인을 완료하지 않은 단원을 잠가야 한다.
- 각 단원 문제 세트는 해당 단원에서 이미 배운 내용만 출제해야 하며 아직 배우지 않은 다른 단원을 섞지 않는다.
- 단원 문제 본문은 선택지 4개, answer 0..3, remediation 정확히 2개를 갖는다. 각 remediation도 선택지 4개와 answer 0..3을 갖는다.
- 단원 문제에서 본문을 틀리면 해당 본문 문제의 보강 2문제를 모두 맞혀야 해당 단원 문제 완료로 인정한다.
- 전범위 drill.html은 모든 시험범위 단원의 개념 확인과 unit practice가 완료되기 전에는 잠겨 있어야 한다. 직접 URL로 열어도 예외가 없어야 한다.
- 단원 문제 완료 상태는 `science-unit-practice-v1:<unitId>`에 저장하고 다음 단원 잠금 판단에 사용한다.
...
- 한 화면에 긴 설명 카드만 연속으로 쌓지 않는다. 핵심은 시각 자료와 짧은 블록으로 보여 주고 선수개념/풀이 루틴 같은 부가 설명은 접을 수 있어야 한다.
- 단원 문제도 텍스트 문제만 연속 배치하지 말고 표·그래프·피스톤·벡터·궤적·입자 모형 등 가능한 시각 자료를 실제 판단 자료로 사용한다.
...
- **10초 회상은 현재 화면의 lesson/check와 같은 개념을 물어야 한다.** 단원 공통 고정 문구를 모든 세부 lesson에 재사용해 현재 확인문제와 어긋나는 회귀를 금지한다.
- 각 객관식 선택지는 1~4 또는 A~D 같은 식별자를 보여 주어 문제와 선지 경계를 분명하게 한다.
